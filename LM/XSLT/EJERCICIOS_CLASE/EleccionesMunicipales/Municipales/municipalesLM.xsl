...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <html>
            ...
            <body><xsl:apply-templates select="municipales"></xsl:apply-templates></body>
        </html>
        
    </xsl:template>
    
    <xsl:template match="municipales">
        
        <xsl:for-each select="escrutinio_sitio">       Resultados de las Elecciones Municipales en :<xsl:value-of select="nombre_sitio"/>        
