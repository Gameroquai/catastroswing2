...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <xsl:element name="catalogo">
            <xsl:for-each-group select="//PLANT" group-by="LIGHT">
                <xsl:variable name="contadorEspecies" select="count(current-group())"/>
                <xsl:element name="grupo">
                    <xsl:attribute name="nombre">
                        <xsl:value-of select="current-grouping-key()"/>
                    </xsl:attribute>
                    <xsl:attribute name="cant_especies">
                        <xsl:value-of select="$contadorEspecies"/>
                    </xsl:attribute>
